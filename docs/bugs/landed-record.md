# BUGS — "Recently landed": full record

Archive of the released-work prose that used to fill over half of `BUGS.md`.
These are shipped and tagged; nothing here is an action item. `BUGS.md` keeps a
one-line index per item so the history is findable without being read.
